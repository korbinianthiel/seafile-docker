...
EXPOSED_SHARE_DIR=${EXPOSED_SHARE_DIR:-"/seafile-share"}
...
setup_exposed_directories() {
    ...
    if [[ -d $SEAFILE_ROOT_DIR/seafile-data ]]; then
       	mv "$SEAFILE_ROOT_DIR/seafile-data" "$EXPOSED_SHARE_DIR"
    else
        mkdir "$EXPOSED_SHARE_DIR/seafile-data"
    fi
    ln -sf "$EXPOSED_SHARE_DIR/seafile-data" "$SEAFILE_ROOT_DIR/seafile-data"
}
